...
function addToDBChange(url, data) {
	$.ajax({
		url : url,
		type : "post",
		data : data,
...
function addToDBComment(url, data) {
    $.ajax({
        url : url,
        type : "post",
        data : data,
...
/*                alert("恭喜您添加评论成功！"+ data.data.list[1].userName + "  " + data.data.list.length);*/
                alert("恭喜您添加评论成功！");

            } else {
                alert("Comment error!" + data.statusMsg);
            }
        },
        error : function(data) {
            alert(JSON.stringify(data));
